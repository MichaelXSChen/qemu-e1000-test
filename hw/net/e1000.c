...
//             /* Pad to minimum Ethernet frame length */
//             if (size < sizeof(min_buf)) {
//                 iov_to_buf(iov_list[pushed_len], 1, 0, min_buf, size);
//                 memset(&min_buf[size], 0, sizeof(min_buf) - size);
//                 e1000x_inc_reg_if_not_full(s->mac_reg, RUC);
//                 min_iov.iov_base = filter_buf = min_buf;
//                 min_iov.iov_len = size = sizeof(min_buf);
//                 iovcnt = 1;
//                 iov_list[pushed_len] = &min_iov;
//             } else if (iov_list[pushed_len]->iov_len < MAXIMUM_ETHERNET_HDR_LEN) {
//                 /* This is very unlikely, but may happen. */
//                 iov_to_buf(iov_list[pushed_len], 1, 0, min_buf, MAXIMUM_ETHERNET_HDR_LEN);
//                 filter_buf = min_buf;
//             }
...
//                 vlan_special = cpu_to_le16(lduw_be_p(filter_buf + 14));
//                 iov_ofs = 4;
//                 if (filter_buf == iov_list[pushed_len]->iov_base) {
//                     memmove(filter_buf + 4, filter_buf, 12);
//                 } else {
//                     iov_from_buf(iov_list[pushed_len], 1, 4, filter_buf, 12);
//                     while (iov_list[pushed_len]->iov_len <= iov_ofs) {
//                         iov_ofs -= iov_list[pushed_len]->iov_len;
//                         iov++;
//                     }
//                 }
//                 vlan_status = E1000_RXD_STAT_VP;
